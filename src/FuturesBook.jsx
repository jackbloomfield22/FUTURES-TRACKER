...
export default function FuturesBook() {
  ...
  const handleFile = useCallback(async (file) => {
    ...
    if (file.type && !file.type.startsWith("image/")) {
      setParseErr("That file isn't an image. Screenshot the slip and try again.");
      return;
    }
    setParsing(true);
    setParseErr("");
    ...
    try {
      ...
      if (!tickets.length) throw new Error("no bets found in that screenshot");
      const drafts = tickets.map((t) => ticketToDraft(t, preview));
      const { fresh, skipped } = dedupeDrafts(drafts);
      if (!fresh.length) {
        setPendingSlip(null);
        setParseErr("Every bet in that screenshot is already in the book, nothing new to add.");
      } else {
        setDraft(fresh[0]);
        setQueue(fresh.slice(1));
        setPendingSlip(null);
        if (skipped) setParseErr(skipped + " duplicate" + (skipped > 1 ? "s" : "") + " already in the book, skipped.");
      }
    } catch (e) {
      const msg = e.message || "unknown error";
      if (preview) setPendingSlip(preview);
      const kept = preview ? " Your slip was captured and will attach to the next ticket you add." : "";
      if (/fetch|network|load failed/i.test(msg)) {
        setParseErr("The AI call can't get through in this view (" + msg + ")." + kept + " Use Paste from chat below, or open this artifact on claude.ai in a desktop browser.");
      } else {
        setParseErr("Couldn't read the slip (" + msg + ")." + kept + " Enter it manually or use Paste from chat.");
      }
    }
    setParsing(false);
  }, [positions]);
        ...
        <section>
          {!draft && !parsing && (
            ...
            >
              ...
              <p className="fb-drop-big">Drop a screenshot of your slip</p>
              <p className="fb-drop-small">or paste it (Ctrl/Cmd+V), or tap to browse. Works on a single slip or a full open-bets list; every bet gets pulled in for you to confirm.</p>
          ...
          {parsing && (
            <div className="fb-drop">
              ...
              <p className="fb-drop-big">Reading the slip…</p>
              <p className="fb-drop-small">Pulling book, market, odds and stake off the screenshot.</p>
            </div>
          )}
          ...
          {draft && !parsing && (
            <div className="fb-form">
              <div className="fb-form-head">
                <h3>{draft.editingId ? "Edit ticket" : "Confirm the ticket"}{queue.length ? ` · ${queue.length} more queued` : ""}</h3>
